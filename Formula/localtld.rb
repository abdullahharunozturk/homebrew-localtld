class Localtld < Formula
  desc "Give your local projects real domains on dynamic ports"
  homepage "https://localtld.sh"
  url "https://github.com/abdullahharunozturk/localtld/archive/refs/tags/v0.1.0.tar.gz"
  sha256 "a7bca12d5960be9403fb7e34bbec008ee987b67db658ec432224d9e539ef46b2"
  license "MIT"
  version "0.1.0"

  depends_on :macos
  depends_on "caddy"
  depends_on "dnsmasq"
  depends_on "jq"

  def install
    bin.install "bin/localtld"
    pkgshare.install "share/Caddyfile.example"
  end

  def caveats
    <<~EOS
      Run first-time setup to configure dnsmasq + Caddy (asks for sudo once):
        localtld setup
    EOS
  end

  test do
    assert_match "0.1.0", shell_output("#{bin}/localtld --version")
  end
end
